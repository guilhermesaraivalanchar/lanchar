class TransferenciaGeraisController < ApplicationController
  ...
  def index
    ...
    @transferencia_gerais = TransferenciaGeral.where("transferencia_gerais.created_at > ? AND transferencia_gerais.created_at < ?", Time.now.beginning_of_day, Time.now.end_of_day).where(tipo: ["VENDA","VENDA_DIRETA","ENTRADA","SAIDA"], escola_id: current_user.escola_id).order("created_at DESC")
  end
